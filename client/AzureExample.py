...
def execute(url, input_path, output_path, filename):
    try:
        ...
        res = requests.post(url,  headers={ 'Accept': 'application/json', 'Content-Type': 'application/json', }, json=raw_data)
...
def main():
    parser = argparse.ArgumentParser(description="Example of posting a request to PDFNet.OfficeToPDF() Azure Function.")
    parser.add_argument('--url',help="URL of PDFNet's Azure functions.", default ='<YOUR PUBLISHED FUNCTION URL>', dest='url')
